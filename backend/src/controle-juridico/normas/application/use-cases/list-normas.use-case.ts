...
@Injectable()
export class ListNormasUseCase {
    constructor(private readonly normaRepository: NormaRepository) {}

    async execute(tenantId: string, query: ListNormasQueryDto) {
        const result = await this.normaRepository.findMany(tenantId, {
            search: query.search,
            tipoId: query.tipoId,
            anoId: query.anoId,
            esferaFederacaoId: query.esferaFederacaoId,
            identificadorId: query.identificadorId,
            materiaOrigemId: query.materiaOrigemId,
            numero: query.numero,
