...
function makeVotacao(encerrada: boolean): VotacaoEntity {
    ...
    v.tipoQuorum = TipoQuorum.MAIORIA_SIMPLES;
    v.totalMembros = 10;
    v.votoQualidade = false;
    v.presidenteId = null;
    return v;
}
...
describe('EncerrarVotacaoUseCase', () => {
    ...
    it('lança NotFoundException quando item de pauta não existe', async () => {
        (prisma.pautaItem.findFirst as jest.Mock).mockResolvedValue(null);
        await expect(useCase.execute('t1', 'sessao-1', 'pauta-1', {}, 'user-1'))
