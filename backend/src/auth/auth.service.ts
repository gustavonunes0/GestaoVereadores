...
import { RoleUsuario, TenantStatus, TenantUserStatus } from '@prisma/client';
...
export class AuthService {
    ...
    async loginCamara(dto: LoginCamaraDto) {
        ...
        const tenant = await this.resolveTenantForCamara(
            dto.tenantId,
            dto.tenantCnpj,
        );
        ...
        if (!tenantUser) {
            throw new UnauthorizedException(
                'Usuário sem vínculo ativo nesta câmara',
            );
        }
    ...
    private async resolveTenantForCamara(
        tenantId?: string,
        tenantCnpj?: string,
    ) {
        if (tenantId) {
