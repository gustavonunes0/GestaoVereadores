  ...
  return (
    <section className="page page--normas">
      ...
      <DataTable
        ...
        onRowClick={(e) => void openDetail((e.data as NormaListItem).id)}
      >
        <Column header="Espécie" body={tipoBody} style={{ width: '11rem' }} />
        ...
        <Column
          header="Publicação"
          body={(row: NormaListItem) => formatDatePt(row.dataPublicacaoInicio ?? row.data)}
          style={{ width: '7rem' }}
        />
      </DataTable>
      )}
      ...
      >
        {detailLoading && <p className="ui-loading-inline">Carregando norma…</p>}
        {detail && !detailLoading && (
          <div className="publicacao-detail">
            <span className="badge-species publicacao-detail__tag">{detail.tipo?.nome ?? 'Norma'}</span>
            <p className="publicacao-detail__ementa">{detail.ementa}</p>
            <PublicacaoMetaGrid
              items={[
                { label: 'Número', value: detail.numero },
                { label: 'Ano', value: detail.ano?.valor ?? '—' },
                { label: 'Data da norma', value: formatDatePt(detail.data) },
                { label: 'Publicação', value: formatDatePt(detail.dataPublicacaoInicio) },
                { label: 'Esfera', value: detail.esferaFederacao?.nome },
                { label: 'Identificador', value: detail.identificador?.nome },
                {
                  label: 'Matéria de origem',
                  value: detail.materiaOrigem?.ementa ?? 'Sem vínculo',
                },
              ]}
            />
            {detail.mensagem && (
      ...
      <Dialog
        ...
        footer={
          <div className="dialog-footer">
            ...
            <SiglButton label="Publicar norma" icon="pi pi-check" loading={saving} onClick={() => void handleCreate()} />
          </div>
        }
      >
        {dominios && (
          <div className="form-stack">
            <div className="form-section">
              <p className="form-section__title">Identificação normativa</p>
              <label htmlFor="norma-tipo" className="field-required">
                Espécie normativa
              </label>
              <Dropdown
                id="norma-tipo"
                ...
                className="w-full"
              />
              <label htmlFor="norma-numero" className="field-required">
                Número
              </label>
              <InputText id="norma-numero" value={numero} onChange={(e) => setNumero(e.target.value)} className="w-full" />
            </div>
            <div className="form-section">
              <p className="form-section__title">Conteúdo e origem legislativa</p>
              <label htmlFor="norma-ementa" className="field-required">
                Ementa
              </label>
              <InputTextarea
                id="norma-ementa"
              ...
              />
              <label htmlFor="norma-materia">Matéria de origem (aprovada)</label>
              <Dropdown
                id="norma-materia"
                ...
                placeholder={
                  materiasAprovadas.length ? 'Selecione a matéria' : 'Nenhuma matéria aprovada'
                }
                disabled={!materiasAprovadas.length}
                className="w-full"
              />
              {!materiasAprovadas.length && (
                <Message severity="warn" text="Aprove uma matéria em tramitação antes de publicar a norma." />
              )}
            </div>
          </div>
        )}
