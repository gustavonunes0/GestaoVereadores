...
export function ModulePipelineFooter({ current }: Props) {
    const sibling =
        current === 'normas'
            ? PUBLICACAO_MODULES.atos
            : PUBLICACAO_MODULES.normas;

    return (
        <footer
            className="module-pipeline-footer"
            aria-label="Outro módulo do fluxo"
        >
            <span className="module-pipeline-footer__label">
                Também no fluxo:
            </span>
            ...
            <span className="module-pipeline-footer__hint">
                {sibling.footerHint}
            </span>
        </footer>
    );
