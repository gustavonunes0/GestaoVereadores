  ...
  return (
    <section className="sessao-filters-card" aria-label={title}>
      <header className="sessao-filters-card__header">
        ...
        >
          <i className={`pi ${expanded ? 'pi-chevron-up' : 'pi-chevron-down'}`} aria-hidden />
          <span>{title}</span>
          {activeCount > 0 && (
            <span className="sessao-filters-card__badge">{activeCount}</span>
          )}
        </button>
        <div className="sessao-filters-card__header-actions">
          ...
          <button type="button" className="btn btn-primary btn-sm" onClick={onPesquisar}>
            Pesquisar
          </button>
        </div>
      </header>
      ...
      {expanded && <div className="sessao-filters-card__body">{children}</div>}
    </section>
  );
}
