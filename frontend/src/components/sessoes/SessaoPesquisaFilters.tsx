...
type Props = {
  ...
  legislaturas: { id: string; numero: number; sessoesLegislativas?: { id: string; numero: number }[] }[];
...
export function SessaoPesquisaFilters({
  ...
  const chips = useMemo(() => {
    ...
    if (periodoModo === 'rapido' && filtros.ano) {
      let p = filtros.ano;
      if (filtros.mes) p += ` · ${MESES_LABEL[filtros.mes] ?? filtros.mes}`;
      if (filtros.dia) p += `/${filtros.dia}`;
      list.push(p);
    }
    if (periodoModo === 'intervalo') {
      if (filtros.dataDe) list.push(`De ${filtros.dataDe.split('-').reverse().join('/')}`);
      if (filtros.dataAte) list.push(`Até ${filtros.dataAte.split('-').reverse().join('/')}`);
    }
    ...
    >
          <div className="sessao-filters-section">
            ...
            <div className="sessao-filters-grid sessao-filters-grid--2">
              ...
              <label className="filter-field">
                <span className="filter-field__label">Sessão legislativa</span>
                <select
                  value={filtros.sessaoLegislativaId}
                  onChange={(e) => onChange({ sessaoLegislativaId: e.target.value })}
                  disabled={!filtros.legislaturaId}
                >
                  <option value="">
                    {filtros.legislaturaId ? 'Todas na legislatura' : 'Selecione a legislatura'}
                  </option>
                  {sessoesLeg.map((s) => (
          ...
          <div className="sessao-filters-section">
            <div className="sessao-filters-section__row">
              <p className="sessao-filters-section__title">Período da sessão</p>
              <div className="segmented" role="tablist" aria-label="Modo de período">
                <button
                  type="button"
                  role="tab"
            ...
            <div className="sessao-filters-presets">
              <button type="button" className="btn btn-ghost btn-sm" onClick={() => aplicarPreset('mes')}>
                Este mês
              </button>
              <button type="button" className="btn btn-ghost btn-sm" onClick={() => aplicarPreset('ano')}>
                Este ano
              </button>
              <button type="button" className="btn btn-ghost btn-sm" onClick={() => aplicarPreset('30dias')}>
                Últimos 30 dias
              </button>
            </div>
            ...
            {periodoModo === 'rapido' ? (
              <div className="sessao-filters-grid sessao-filters-grid--3">
                <label className="filter-field">
                  ...
                  <select value={filtros.ano} onChange={(e) => onChange({ ano: e.target.value })}>
                ...
                <label className="filter-field">
                  ...
                  >
                    {MESES_OPCOES.map((m) => (
                      <option key={m.value || 'v'} value={m.value}>
                        {m.value ? MESES_LABEL[m.value] ?? m.label : 'Qualquer'}
                      </option>
                    ))}
                  </select>
                </label>
              ...
              <div className="sessao-filters-grid sessao-filters-grid--2">
                <label className="filter-field">
                  <span className="filter-field__label">Data início (de)</span>
                  <input
                    type="date"
                ...
                <label className="filter-field">
                  <span className="filter-field__label">Data início (até)</span>
                  <input
                    type="date"
          ...
          <div className="sessao-filters-section">
            ...
            <div className="sessao-filters-grid sessao-filters-grid--2">
              <label className="filter-field">
                <span className="filter-field__label">Tipo de sessão</span>
                <select
                  value={filtros.tipoSessaoId}
                  onChange={(e) => onChange({ tipoSessaoId: e.target.value })}
                >
              ...
              <label className="filter-field">
                ...
                <select
                  ...
                  onChange={(e) => onChange({ situacaoId: e.target.value })}
                >
