...
export function StatsBlock({ stats }: { stats: PanelStats }) {
    return (
        <div
            ...
            aria-live="polite"
            aria-atomic="true"
            aria-label={`Parlamentares ${stats.parlamentares}, ausentes ${stats.ausentes}, presentes ${stats.presentes}`}
        >
            ...
            <div className="vp-stats__row vp-stats__row--presentes" aria-hidden>
                <span>PRESENTES</span>
                <strong>{String(stats.presentes).padStart(2, '0')}</strong>
            </div>
        </div>
    );
}
