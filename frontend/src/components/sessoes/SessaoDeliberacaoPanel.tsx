...
export function SessaoDeliberacaoPanel({
  ...
  const [abrirVotacaoItemId, setAbrirVotacaoItemId] = useState<string | null>(null);
  ...
  const [votoValor, setVotoValor] = useState<(typeof VOTO_OPTIONS)[number]['value']>('SIM');
  ...
  async function handleRegistrarVoto(pautaItemId: string) {
    ...
    setBusy(true);
    try {
      await api(`/sessoes/${sessaoId}/pauta/${pautaItemId}/votacao/votos`, {
        method: 'POST',
        body: JSON.stringify({
  ...
  async function handleFinalizar(pautaItemId: string, votacao: Votacao) {
    ...
    if (votacao.tipoVotacao === 'SIMBOLICA' || votacao.tipoVotacao === 'SECRETA') {
      body.votosSim = votosSim;
      body.votosNao = votosNao;
      body.abstencoes = abstencoes;
    }
    try {
      await api(`/sessoes/${sessaoId}/pauta/${pautaItemId}/votacao/finalizar`, {
        method: 'PATCH',
        body: JSON.stringify(body),
      });
      showSuccess('Votação finalizada. Resultado aplicado à pauta e à matéria quando aplicável.');
      onUpdated();
    } catch (err) {
      showApiError(err);
    } finally {
  ...
  return (
    <>
        ...
        <ul className="presenca-list">
          {presencas.map((p, i) => (
            <li key={p.parlamentarId ?? p.parlamentar?.id ?? i}>
              ...
              {p.presente || p.situacao === 'PRESENTE' ? 'Presente' : 'Ausente'}
            </li>
          ))}
        </ul>
      ) : (
      ...
      {!pautaItens.length ? (
        <p className="muted">
          Pauta vazia — inclua matérias com status EM_TRAMITACAO enquanto a sessão estiver em
          andamento.
        </p>
      ) : (
        <ol className="pauta-list pauta-list--deliberacao">
          {pautaItens.map((item) => {
            ...
            const hideVotes = votacao ? shouldHideNominalVotes(votacao.tipoVotacao) : false;

            return (
              <li key={item.id} className="pauta-item-deliberacao">
                <div className="pauta-item-deliberacao__head">
                  <span className="pauta-ordem">{item.ordem}</span>
                  <div>
                    <div>{item.materia?.ementa ?? '—'}</div>
                    {item.resultado && (
                      <span className="badge badge-muted">Pauta: {item.resultado}</span>
                    )}
                  </div>
                </div>
                ...
                {votacao && aberta && (
                  <div className="pauta-item-deliberacao__votacao">
                    <p className="muted">
                      Votação {votacao.tipoVotacao.toLowerCase()} em andamento
                    </p>

                    {!hideVotes && votacao.votos && votacao.votos.length > 0 && (
                      <ul className="presenca-list">
                        {votacao.votos.map((v) => (
                          <li key={v.id}>
                            {v.parlamentar?.pessoa?.nome ?? v.parlamentarId}: {v.voto}
                          </li>
                        ))}
                      </ul>
                    )}
                    ...
                    {hideVotes && (
                      <p className="alert-warn">
                        Votação secreta: a interface não exibe votos nominais individuais.
                      </p>
                    )}

                    {podeDeliberar && votacao.tipoVotacao === 'NOMINAL' && (
                      <div className="sigl-cluster sigl-cluster--loose">
                        <select
                          value={votoParlamentarId}
                          onChange={(e) => setVotoParlamentarId(e.target.value)}
                        >
                          {parlamentares.map((p) => (
                        ...
                        <select
                          value={votoValor}
                          onChange={(e) =>
                            setVotoValor(e.target.value as (typeof VOTO_OPTIONS)[number]['value'])
                          }
                        >
                          {VOTO_OPTIONS.map((o) => (
                        ...
                        <SiglButton
                          ...
                          icon="pi pi-check"
                          loading={busy}
                          onClick={() => void handleRegistrarVoto(item.id)}
                        />
                      </div>
                    )}
                        ...
                        <div className="form-grid">
                          <label>
                            ...
                            <input
                              ...
                              min={0}
                              value={votosSim}
                              onChange={(e) => setVotosSim(Number(e.target.value))}
                            />
                          </label>
                          <label>
                            Votos não
                            <input
                              ...
                              min={0}
                              value={votosNao}
                              onChange={(e) => setVotosNao(Number(e.target.value))}
                            />
                          </label>
                          <label>
                            Abstenções
                            <input
                              ...
                              min={0}
                              value={abstencoes}
                              onChange={(e) => setAbstencoes(Number(e.target.value))}
                            />
                          </label>
                        </div>
                      )}
                    ...
                    {podeDeliberar && (
                      <SiglButton
                        ...
                        severity="success"
                        loading={busy}
                        onClick={() => void handleFinalizar(item.id, votacao)}
                      />
                    )}
                  </div>
                )}
                  ...
                  <div className="sigl-cluster">
                    {abrirVotacaoItemId === item.id ? (
                      <>
                        <select
                          value={tipoVotacao}
                          onChange={(e) => setTipoVotacao(e.target.value as TipoVotacao)}
                        >
                          {TIPO_VOTACAO_OPTIONS.map((o) => (
                            <option key={o.value} value={o.value}>
                              {o.label}
                            </option>
                          ))}
                        ...
                        <SiglButton
                          ...
                          icon="pi pi-play"
                          loading={busy}
                          onClick={() => void handleAbrirVotacao(item.id)}
                        />
                        <SiglButton
                          label="Cancelar"
                          severity="secondary"
                          text
                          onClick={() => setAbrirVotacaoItemId(null)}
                        />
                      </>
                    ) : (
                      <SiglButton
                        ...
                        icon="pi pi-play"
                        outlined
                        onClick={() => setAbrirVotacaoItemId(item.id)}
                      />
                    )}
                  </div>
                )}
      ...
      {presencaOpen && (
        <Modal title="Registrar presença" onClose={() => !busy && setPresencaOpen(false)}>
