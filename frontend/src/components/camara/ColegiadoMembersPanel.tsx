...
export function ColegiadoMembersPanel({
  ...
  useEffect(() => {
    ...
    apiList<ParlamentarOption>('/parlamentares', { limit: 100 })
      ...
      .catch((err: unknown) => {
        ...
        setParlamentarId('');
        setLoadError(
          err instanceof Error ? err.message : 'Não foi possível carregar parlamentares.',
        );
      })
  ...
  return (
    <>
      ...
      {membros.length === 0 ? (
        <p className="muted">Nenhum membro na composição. Adicione parlamentares e cargos.</p>
      ) : (
          ...
          {membros.map((m) => (
            <li key={m.id} className="membro-mesa-item">
              <div>
                ...
                <span>{m.parlamentar?.pessoa?.nome ?? '—'}</span>
                {showTitular && (
                  <span className="badge badge-muted" style={{ marginLeft: '0.35rem' }}>
                    {m.titular !== false ? 'Titular' : 'Suplente'}
                  </span>
                )}
              </div>
              {canWrite && (
      ...
      {open && (
        <Modal title={`Adicionar membro — ${entityLabel}`} onClose={() => setOpen(false)}>
          <form onSubmit={handleAdd}>
            <label>
              Parlamentar *
              <select
                value={parlamentarId}
                onChange={(e) => setParlamentarId(e.target.value)}
                required
                disabled={loadingParlamentares || parlamentares.length === 0}
              >
            ...
            {!loadError && !loadingParlamentares && parlamentares.length === 0 && (
              <p className="muted" style={{ fontSize: '0.85rem' }}>
                Cadastre vereadores em Estrutura da Câmara → Parlamentares.
              </p>
            )}
              ...
              <label>
                ...
                >
                  {dominios.cargosMesa.map((c) => (
                    <option key={c.id} value={c.id} disabled={cargosOcupados.has(c.id)}>
                      {c.nome}
                      {cargosOcupados.has(c.id) ? ' (ocupado)' : ''}
                    </option>
                  ))}
                </select>
              </label>
