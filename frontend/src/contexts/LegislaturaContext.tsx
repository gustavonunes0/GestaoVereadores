...
function loadStored(): { legislaturaId: string; sessaoLegislativaId: string } {
  try {
    ...
    if (raw) return JSON.parse(raw) as { legislaturaId: string; sessaoLegislativaId: string };
  } catch {
    /* ignore */
  }
...
export function LegislaturaProvider({ children }: { children: ReactNode }) {
  ...
  const [legislaturaId, setLegislaturaIdState] = useState(() => loadStored().legislaturaId);
  const [sessaoLegislativaId, setSessaoLegislativaIdState] = useState(
    () => loadStored().sessaoLegislativaId,
  );

  const refresh = useCallback(async () => {
    const res = await apiList<LegislaturaRef>('/legislaturas', { limit: 20 });
  ...
  useEffect(() => {
    refresh()
      .then((list) => {
        ...
        let legId =
          stored.legislaturaId && list.some((l) => l.id === stored.legislaturaId)
  ...
  const sessaoLegislativaAtiva = useMemo(
    () =>
      legislaturaAtiva?.sessoesLegislativas?.find((s) => s.id === sessaoLegislativaId) ??
      null,
  ...
  return (
    <LegislaturaContext.Provider value={value}>{children}</LegislaturaContext.Provider>
  );
}
